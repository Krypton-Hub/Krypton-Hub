...
local function semiInvisibleFunction()
    local UNDERGROUND_OFFSET = -5  -- Real HRP goes 5 studs below ground
    local TRANSPARENCY = 1  -- Full transparency for character parts
    local CLONE_TRANSPARENCY = 0.9  -- Near-transparent for LowerTorso clone
    ...
    local function removeFolders()
        ...
        for _, name in pairs({"DoubleRig", "Constraints"}) do
            local folder = playerFolder:FindFirstChild(name)
    ...
    local function makeTransparent()
        ...
        for _, part in pairs(character:GetDescendants()) do
            if part:IsA("BasePart") and part ~= clone then
                originalProperties[part] = originalProperties[part] or {
            ...
            elseif part:IsA("Decal") then
                originalProperties[part] = originalProperties[part] or { Transparency = part.Transparency }
                part.Transparency = TRANSPARENCY
            end
        end
        if clone then
            clone.Transparency = CLONE_TRANSPARENCY  -- Near-invisible but functional
    ...
    local function doClone()
        ...
        clone = torso:Clone()  -- Clone LowerTorso or Torso
        clone.Parent = character
        clone.Transparency = CLONE_TRANSPARENCY
        clone.CanCollide = true
        oldRoot.Parent = Workspace
        clone.CFrame = oldRoot.CFrame  -- Match initial position

        character.PrimaryPart = clone
        character.Parent = Workspace

        -- Update welds to reference the clone
        for _, v in pairs(character:GetDescendants()) do
            if v:IsA("Weld") or v:IsA("Motor6D") or v:IsA("WeldConstraint") then
                if v.Part0 == oldRoot then v.Part0 = clone end
                if v.Part1 == oldRoot then v.Part1 = clone end
                if v.Part0 == torso then v.Part0 = clone end
                if v.Part1 == torso then v.Part1 = clone end
            end
        end
    ...
    local function revertClone()
        ...
        if clone then
            ...
            clone:Destroy()
            clone = nil
            oldRoot.CFrame = oldPos  -- Return HRP to clone's position (above ground)
        end
    ...
    local function enableInvisibility()
        ...
        connection = RunService.Heartbeat:Connect(function()
            pcall(function()
                if character and humanoid and humanoid.Health > 0 and oldRoot and clone then
                    local root = character.PrimaryPart or clone
                    if root then
        ...
        characterConnection = player.CharacterAdded:Connect(function(newChar)
            if isInvisible then
                ...
                statusLabel.Text = "Full Invisible disabled (respawn)"
...
-- Connect semi-invisible to button
